...
#define KV_NAME_LEN_OFFSET                       ((unsigned long)(&((struct kv_hdr_data *)0)->value_len))
...
static void update_kv_cache(fdb_kvdb_t db, uint32_t key, uint32_t addr)
{
    size_t i, empty_index = FDB_KV_CACHE_TABLE_SIZE, min_activity_index = FDB_KV_CACHE_TABLE_SIZE;
    uint16_t name_crc = (uint16_t) (fdb_calc_crc32(0, (void *)&key, sizeof(uint32_t)) >> 16), min_activity = 0xFFFF;
 ...
 */
static bool get_kv_from_cache(fdb_kvdb_t db, uint32_t key, uint32_t *addr)
{
    size_t i;
    uint16_t name_crc = (uint16_t) (fdb_calc_crc32(0, (void *)&key, sizeof(uint32_t)) >> 16);

    for (i = 0; i < FDB_KV_CACHE_TABLE_SIZE; i++) {
        if ((db->kv_cache_table[i].addr != FDB_DATA_UNUSED) && (db->kv_cache_table[i].name_crc == name_crc)) {
            uint32_t saved_key;
            /* read the KV name in flash */
            _fdb_flash_read((fdb_db_t)db, db->kv_cache_table[i].addr + KV_HDR_DATA_SIZE, (uint32_t *) &saved_key, sizeof(uint32_t));
            if (saved_key == key) {
...
static fdb_err_t read_kv(fdb_kvdb_t db, fdb_kv_t kv)
{
    ...
    if (calc_crc32 != kv_hdr.crc32) {
        ...
        _fdb_flash_read((fdb_db_t)db, kv_name_addr, (uint32_t *) (void *)&kv->key, FDB_WG_ALIGN(sizeof(uint32_t)));
        /* the value is behind aligned name */
        kv->addr.value = kv_name_addr + FDB_WG_ALIGN(sizeof(uint32_t));
...
{
    const uint32_t *key = arg1;
    bool *find_ok = arg2;

    /* check KV */
    if (kv->crc_is_ok && kv->status == FDB_KV_WRITE && (kv->key == *key)) {
        *find_ok = true;
        return true;
    }
    return false;
...
static bool find_kv_no_cache(fdb_kvdb_t db, uint32_t key, fdb_kv_t kv)
{
    bool find_ok = false;
...
static bool find_kv(fdb_kvdb_t db, uint32_t key, fdb_kv_t kv)
{
    bool find_ok = false;

#ifdef FDB_KV_USING_CACHE
    if (get_kv_from_cache(db, key, &kv->addr.start)) {
        read_kv(db, kv);
        return true;
    }
#endif /* FDB_KV_USING_CACHE */
...
static size_t get_kv(fdb_kvdb_t db, uint32_t key, void *value_buf, size_t buf_len, size_t *value_len)
{
    struct fdb_kv kv;
    size_t read_len = 0;
...
static fdb_err_t del_kv(fdb_kvdb_t db, uint32_t key, fdb_kv_t old_kv, bool complete_del)
{
    fdb_err_t result = FDB_NO_ERR;
    ...
    if (!old_kv) {
        ...
        if (find_kv(db, key, &kv)) {
            ...
            FDB_DEBUG("Not found '0x%08x' in KV.\n", key);
...
static fdb_err_t move_kv(fdb_kvdb_t db, fdb_kv_t kv)
{
    ...
    if ((kv_addr = alloc_kv(db, &sector, kv->len)) != FAILED_ADDR) {
        if (db->in_recovery_check) {
            ...
            if (find_kv_no_cache(db, kv->key, &kv_bak)) {
    ...
    /* start move the KV */
    {
        ...
        update_sector_cache(db, FDB_ALIGN_DOWN(kv_addr, db_sec_size(db)),
                kv_addr + KV_HDR_DATA_SIZE + FDB_WG_ALIGN(sizeof(uint32_t)) + FDB_WG_ALIGN(kv->value_len));
        update_kv_cache(db, kv->key, kv_addr);
#endif /* FDB_KV_USING_CACHE */
    }

    FDB_DEBUG("Moved the KV (0x%08x) from 0x%08" PRIX32 " to 0x%08" PRIX32 ".\n", kv->key, kv->addr.start, kv_addr);
        ...
        do {
            ...
            if (kv.crc_is_ok && (kv.status == FDB_KV_WRITE || kv.status == FDB_KV_PRE_DELETE)) {
                ...
                if (move_kv(db, &kv) != FDB_NO_ERR) {
                    FDB_DEBUG("Error: Moved the KV (0x%08x) for GC failed.\n", kv.key);
                }
            }
        } while ((kv.addr.start = get_next_kv_addr(db, sector, &kv)) != FAILED_ADDR);
...
static fdb_err_t create_kv_blob(fdb_kvdb_t db, kv_sec_info_t sector, uint32_t key, const void *value, size_t len)
{
    fdb_err_t result = FDB_NO_ERR;
    struct kv_hdr_data kv_hdr;
    bool is_full = false;
    ...
    kv_hdr.len = KV_HDR_DATA_SIZE + FDB_WG_ALIGN(sizeof(uint32_t)) + FDB_WG_ALIGN(kv_hdr.value_len);
    ...
    if (kv_addr != FAILED_ADDR || (kv_addr = new_kv(db, sector, kv_hdr.len)) != FAILED_ADDR) {
        ...
        if (result == FDB_NO_ERR) {
            ...
            kv_hdr.crc32 = fdb_calc_crc32(0, &kv_hdr.value_len, KV_HDR_DATA_SIZE - KV_NAME_LEN_OFFSET);
            kv_hdr.crc32 = fdb_calc_crc32(kv_hdr.crc32, (void *)&key, sizeof(uint32_t));
            align_remain = FDB_WG_ALIGN(sizeof(uint32_t)) - sizeof(uint32_t);
            while (align_remain--) {
        ...
        /* write key name */
        if (result == FDB_NO_ERR) {
            result = align_write(db, kv_addr + KV_HDR_DATA_SIZE, (uint32_t *) &key, sizeof(uint32_t));

#ifdef FDB_KV_USING_CACHE
            if (!is_full) {
                update_sector_cache(db, sector->addr,
                        kv_addr + KV_HDR_DATA_SIZE + FDB_WG_ALIGN(sizeof(uint32_t)) + FDB_WG_ALIGN(kv_hdr.value_len));
            }
        ...
        /* write value */
        if (result == FDB_NO_ERR) {
            result = align_write(db, kv_addr + KV_HDR_DATA_SIZE + FDB_WG_ALIGN(sizeof(uint32_t)), value,
                    kv_hdr.value_len);
        }
...
static fdb_err_t set_kv(fdb_kvdb_t db, uint32_t key, const void *value_buf, size_t buf_len)
{
    fdb_err_t result = FDB_NO_ERR;
    ...
    if (value_buf == NULL) {
        ...
        if (new_kv_ex(db, &db->cur_sector, sizeof(uint32_t), buf_len) == FAILED_ADDR) {
            return FDB_SAVED_FULL;
        }
...
static bool check_and_recovery_kv_cb(fdb_kv_t kv, void *arg1, void *arg2)
{
    ...
    if (kv->crc_is_ok && kv->status == FDB_KV_PRE_DELETE) {
        FDB_INFO("Found an KV (0x%08x) which has changed value failed. Now will recovery it.\n", kv->key);
        /* recovery the old KV */
        if (move_kv(db, kv) == FDB_NO_ERR) {
        ...
        update_kv_cache(db, kv->key, kv->addr.start);
    }
